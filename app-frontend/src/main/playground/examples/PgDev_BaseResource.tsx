...
import {AdapterBuilder, AppPage, DropDown, NodeRendererProps, ToastModule, TS_TextArea} from "@nu-art/thunderstorm/frontend";
...
export abstract class PgDev_BaseResource<_State extends BaseResource_State>
	...
	private renderColorsDropDown() {
		const adapter = AdapterBuilder()
			.list()
			.singleRender(
				(props: NodeRendererProps<keyof ColorsType>) => (
			...
			.setData(_keys(COLORS))
			.build();

		return <div className="ll_h_c" style={{minHeight: "30px"}}>
			<div style={{minWidth: "100px", marginRight: "16px"}}>Select a color:</div>
			<DropDown<keyof ColorsType> adapter={adapter} onSelected={selected => this.setState({color: selected})}
